...
namespace RTT
{
    public class Telnet
    {
        ...
        #endregion
        public delegate void write_to_remote(string text);
        

        public write_to_remote writereceive;
        ...
        /// 启动socket 进行telnet操作
        /// </summary>
        public void doSocket(string remoteaddr,int remoteport)
        {
            //获得链接的地址,可以是网址也可以是IP
            //Console.Write("Server Address:");
            //解析输入,如果是一个网址,则解析成ip

            //IPAddress IPadr = IPAddress.Parse(address);//先把string类型转换成IPAddress类型

            //s = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);

            //iep = new System.Net.IPEndPoint(IPadr, port);//传递IPAddress和Port


            IPAddress import = IPAddress.Parse(remoteaddr);// GetIP(Console.ReadLine());
            //获得端口号
            ...
            */
            ...
            s.BeginConnect(
                 ...
                 */
                {
                    ...
                    {
                        writereceive("初始化接收信息出错:" + ex.Message);
        ...
        private void OnRecievedData(IAsyncResult ar)
        {
            ...
            {
                // 输出   关闭连接
                writereceive("Disconnected"+ sock.RemoteEndPoint);
                //Console.WriteLine("Disconnected", sock.RemoteEndPoint);
                // 关闭socket
                sock.Shutdown(SocketShutdown.Both);
                sock.Close();
                writereceive("receive over.");
        ...
        private void RespondToOptions()
        {
            ...
            {
                writereceive("出错了,在回发数据的时候 " + ers.Message);
